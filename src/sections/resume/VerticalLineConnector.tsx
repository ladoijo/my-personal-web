...
const VerticalLineConnector = ({ type }: { type: 'work' | 'education' }) => (
  <div className="relative flex h-full flex-col items-center">
    <div className="z-11 flex h-7 w-7 items-center justify-center rounded-full border-2 bg-emerald-500">
      {type === 'work' ? (
        <BriefcaseIcon className="h-4 w-4 items-center justify-center text-white" />
      ) : (
        <GraduationCapIcon className="h-4 items-center justify-center text-white" />
      )}
    </div>
